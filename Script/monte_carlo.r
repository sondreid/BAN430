...
geny <- function(fit, n, b = 24) {
    ...
    y <- augment(arima_fit)$.fitted
    plot(y, type = "l")


    return(lines(cumsum(x), type = "l"))
}
...
for(i in 1:R){
    y <- geny(arima_fit, n + h)
    x[1] <- 0
    for (j in 1:R) {
         x[j] <- 0.5*y[j] + 0.5*x[j-1]
    }
    x <- rt(n, df =v)
    var_multi_model <- model(var_xt = fable::VAR(vars(y, x), ic = "bic"))
    #### Forecast
    # res[1,2] <- forecast_error
}
